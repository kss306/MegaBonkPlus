...
public class HttpServer
{
        ...
        try
        {
            ...
            while (_isRunning)
            {
                ...
                if (context.Request.HttpMethod == "GET")
                {
                    ...
                    {
                        ModLogger.LogError(
                            $"[HttpServer] Static file handler crashed for {context.Request.Url.AbsolutePath}: {ex}");
                    }
                }
                else
                {
    ...
    private void TryOpenBrowser()
    {
        ...
        var url = $"http://localhost:{ModConfig.WebServerPort.Value}/";
        ...
        try
        {
            ...
            ModLogger.LogInfo($"Opened browser at {url}");
        }
        catch (Exception ex)
        {
